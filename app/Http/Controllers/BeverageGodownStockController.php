...
class BeverageGodownStockController extends Controller
{
    ...
    {
        try {
            $page_title = 'Beverage Godown Stock Management';
            $title      = 'Beverage Godown Stock List';
            ...
            return view('beverage_stock.godown.list', compact(
                'beverageItems', 'stockMap', 'pendingItemIds',
    ...
    private function getReportData(Request $request): array
    {
        ...
        $reportData = $beverageItems->map(function ($item) use ($inDuring, $outDuring, $transferDuring, $inAmountDuring, $wac, $currentStockMap, $useLiveClosing, $closingInUpToTo, $closingOutUpToTo) {
    ...
    public function report(Request $request)
    {
        try {
            ...
            $page_title = 'Beverage Godown Stock Report';
            $title      = 'Beverage Godown Report';

            return view('beverage_stock.godown.report', array_merge($data, compact('page_title', 'title')));
        } catch (\Throwable $th) {
            return $th->getMessage();
        }
    }
    ...
    public function downloadReport(Request $request)
    {
        try {
            $data     = $this->getReportData($request);
            $pdf      = Pdf::loadView('beverage_stock.godown.report_pdf', $data)
                ->setPaper('a4', 'landscape');
            $filename = 'beverage-godown-report-' . $data['from']->format('d-m-Y') . '-to-' . $data['to']->format('d-m-Y') . '.pdf';
