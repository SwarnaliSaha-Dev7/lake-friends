...
@php
    $totalItems    = $beverageItems->count();
    $inStockCount  = 0;
    $lowStockCount = 0;
    $outOfStock    = 0;
...
@endphp
    ...
    @php
        $alertItems = $beverageItems->filter(function ($item) use ($barStockMap) {
            $barStock  = $barStockMap[$item->id] ?? null;
            $barQty    = $barStock ? (int) $barStock->quantity : 0;
            $alertQty  = (int) ($item->low_stock_alert_qty ?? 0);
            $isBeer    = (bool) $item->is_beer;
        ...
        <div class="col-12">
            <div class="member-list-part position-relative">
                <div class="d-flex align-items-center justify-content-between gap-2 mb-3 flex-wrap">
                    ...
                    <a href="{{ route('beverage-bar-stock.report') }}" class="btn btn-outline-info btn-sm fw-semibold">
                ...
                <div class="table-responsive">
                    <table class="table rounded-3 overflow-hidden clubmemberlist2" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            @forelse($beverageItems as $index => $item)
                                @php
                                    $barStock    = $barStockMap[$item->id]    ?? null;
...
$(document).ready(function () {
    ...
    $('#transferForm').on('submit', function (e) {
        ...
        $.ajax({
            url: '{{ route("beverage-bar-stock.transfer") }}',
            method: 'POST',
            data: {
                _token:        '{{ csrf_token() }}',
