    ...
    function renderSessionModal(session, pendingTotal, walletBalance, cardBalanceInfo, minimumUsageInfo) {
        ...
        html += '<div class="p-3 bg-light border rounded-3 mt-2">'
            ...
            + (sessionFoodSubtotal > 0
                ? '<div class="row mb-1 border-bottom pb-1"><div class="col-8 text-end text-muted small">Food GST ({{ $globalRestaurantGstPercentage }}%)</div><div class="col-4 text-center fw-semibold small">Rs ' + dispFoodGst.toFixed(2) + '</div></div>'
                : '')
            + (dispBevGst > 0
                ? '<div class="row mb-1 border-bottom pb-1"><div class="col-8 text-end text-muted small">Beverage GST ({{ $globalBeverageGstPercentage }}%)</div><div class="col-4 text-center fw-semibold small">Rs ' + dispBevGst.toFixed(2) + '</div></div>'
                : '')
