...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .s-delivered { background: #ede9fe; color: #4f46e5; border: 1px solid #a5b4fc; }
        ...
        .s-refunded  { background: #e0f2fe; color: #0369a1; border: 1px solid #7dd3fc; }
...
<body>
    ...
    <table style="width:100%; border-collapse:collapse; margin-bottom:14px;">
        <tr>
            ...
            <td style="width:30%; text-align:right; vertical-align:middle;">
                ...
                <div style="font-size:10px; color:#6c757d; margin-top:2px;">{{ $order->order_no }}</div>
            </td>
        </tr>
    </table>
    ...
    {{-- ===== Top meta: same 2-col layout as modal ===== --}}
    <table class="meta-table">
        <tr>
            <td class="meta-left">
                <div class="meta-label">Order No: <span class="meta-value">{{ $order->order_no }}</span></div>
                <div class="meta-label" style="margin-top:3px;">Bill No: <span class="meta-value">{{ $order->bill_no ?? '—' }}</span></div>
                <div style="margin-top:3px;">
                    <span class="meta-label">Date: <span class="meta-value">{{ $orderDate }}</span></span>
                    &nbsp;&nbsp;
                    <span class="meta-label">Status: </span>
                    <span class="status-pill {{ $statusPillClass }}">{{ ucfirst($order->status) }}</span>
                </div>
            </td>
            <td class="meta-right">
            ...
            <tbody>
                @foreach($liquorItems as $item)
                    @php
                        ...
                        $volume    = $item->metadata['volume_ml'] ?? ($item->metadata['volume'] ?? '—');
    ...
    @php
        ...
        $dispFoodGst      = $rawGstSum > 0 ? $rawFoodGst * ($order->gst_amount / $rawGstSum) : 0;
        $dispBeverageGst  = $rawGstSum > 0 ? $rawBeverageGst * ($order->gst_amount / $rawGstSum) : 0;
    @endphp
    <div class="bill-box">
        <table class="bill-row-table">
            <tr>
                <td class="bill-lbl">Subtotal</td>
                <td class="bill-val">Rs {{ number_format($order->taxable_amount, 2) }}</td>
            </tr>
            @if($foodSubtotal > 0)
            <tr>
            ...
            <tr>
                ...
                <td class="bill-offer-val">-Rs {{ number_format($order->discount_amount, 2) }}</td>
            </tr>
            <tr class="grand-total-row">
                <td class="bill-lbl">Grand Total</td>
                <td class="bill-val" style="font-size:14px;">Rs {{ number_format($order->net_amount, 2) }}</td>
            </tr>
        </table>
    </div>
