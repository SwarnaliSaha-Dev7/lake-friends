        ...
        <div class="col-12">
            <div class="member-list-part position-relative">
                <div class="d-flex align-items-center justify-content-between gap-2 mb-2 flex-wrap">
                    <h2 class="fs-5 common-heading mb-md-0 fw-semibold">Misc Approval List</h2>
                </div>
                <div class="table-responsive">
                    <table class="table rounded-3 overflow-hidden clubmemberlist2" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            @forelse ($miscApprovalData as $data)
                                @php
                                ...
                                <tr>
                                    ...
                                    <td class="text-nowrap fw-medium">{{ $data->entity->name ?? ($payload->item_name ?? ($payload->name ?? '—')) }}</td>
...
$(document).ready(function () {
    ...
    $('#confirmApproveBtn').on('click', function () {
        ...
        $.ajax({
            url: '{{ route("miscApproval.approve", ":id") }}'.replace(':id', id),
            type: 'GET',
    ...
    $('#confirmRejectBtn').on('click', function () {
        ...
        $.ajax({
            url: '{{ route("miscApproval.reject", ":id") }}'.replace(':id', id),
            type: 'GET',
            success: function (response) {
                if (response.statusCode == 200) {
                    toastr.success(response.message);
