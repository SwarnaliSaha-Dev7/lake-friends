        ...
        <div class="col-12">
            <div class="member-list-part position-relative">
                <div class="d-flex align-items-center justify-content-between gap-2 mb-2 flex-wrap">
                    <h2 class="fs-5 common-heading mb-md-0 fw-semibold">Beverage Approval List</h2>
                </div>
                <div class="table-responsive">
                    <table class="table rounded-3 overflow-hidden clubmemberlist2" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            @forelse ($beverageApprovalData as $data)
                                @php
                                    ...
                                    $actionLabel = match($data->module) {
                                        'beverage_item_create' => 'Add Item',
                                        'beverage_item_delete' => 'Delete Item',
                                        'beverage_price_update' => 'Price Change',
                                        default => ucfirst(str_replace('_', ' ', $data->module)),
                                    };

                                    $actionBadgeClass = match($data->module) {
                                        'beverage_item_create'  => 'bg-success-subtle text-success border-success',
                                        'beverage_item_delete'  => 'bg-danger-subtle text-danger border-danger',
                                        'beverage_price_update' => 'bg-info-subtle text-info border-info',
                                        default               => 'bg-secondary-subtle text-secondary border-secondary',
                                    };
                                @endphp
                                <tr>
                                    <td class="text-nowrap">{{ $loop->iteration }}</td>
                                    <td class="text-nowrap fw-medium">{{ $data->entity->name ?? ($payload->name ?? '—') }}</td>
                                    <td class="text-nowrap">
                                        <span class="badge border rounded-pill px-3 py-1 {{ $actionBadgeClass }}">{{ $actionLabel }}</span>
                                    </td>
                                    <td class="text-nowrap small text-muted">
                                        @if($data->module === 'beverage_price_update')
...
$(document).ready(function () {
    ...
    $('#confirmApproveBtn').on('click', function () {
        ...
        $.ajax({
            url: '{{ route("beverageApproval.approve", ":id") }}'.replace(':id', id),
            type: 'GET',
    ...
    $('#confirmRejectBtn').on('click', function () {
        ...
        $.ajax({
            url: '{{ route("beverageApproval.reject", ":id") }}'.replace(':id', id),
            type: 'GET',
            success: function (response) {
                if (response.statusCode == 200) {
                    toastr.success(response.message);
