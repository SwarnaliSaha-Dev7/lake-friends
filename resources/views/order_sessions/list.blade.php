        ...
        <div class="col-12">
            <div class="member-list-part position-relative">
                ...
                <div class="d-flex align-items-center justify-content-between gap-2 mb-3 flex-wrap">
                    <h2 class="fs-5 common-heading mb-0 fw-semibold">Today's Order</h2>
                                ...
                                <tr id="session-row-{{ $session->id }}">
                                    ...
                                    <td class="text-nowrap">
                                        ...
                                        <button class="border-0 bg-light p-1 rounded-3 lh-1 action-btn view-session-btn"
                                            data-id="{{ $session->id }}" title="View order details">
        ...
        <div class="modal-dialog modal-dialog-centered" style="max-width:400px;">
            <div class="modal-content rounded-3 border-0 shadow">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title fw-semibold">Open New Order</h5>
                ...
                <div class="modal-footer border-0 pt-0">
                    ...
                    <button class="btn btn-primary" id="confirmOpenSessionBtn">Open Order</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered" style="max-width:380px;">
            <div class="modal-content rounded-3 border-0 shadow">
                <div class="modal-body text-center p-4">
                    ...
                    <h5 class="fw-semibold mb-1" id="cancelSessionTitle">Cancel Order?</h5>
...
$(document).ready(function () {
    ...
    $(document).on('click', '.cancel-session-btn', function () {
        ...
        if (isBilled) {
            ...
            $('#cancelSessionDesc').text('This will cancel the order and refund');
            $('#cancelSessionRefund').text(amount).removeClass('d-none');
        } else {
            $('#cancelSessionTitle').text('Cancel Order?');
    ...
    function renderSessionModal(session, pendingTotal, walletBalance, cardBalanceInfo, minimumUsageInfo) {
        ...
        if (!orders.length) {
            ...
            orders.forEach(function (order) {
                ...
                items.forEach(function (it) {
                    ...
                    if (it.offer_applied && !isCancelled) {
                        ...
                        if (of.type_slug === 'b1g1') {
                            offerBadge = ' <span class="badge bg-warning-subtle text-warning border border-warning rounded-pill px-2" style="font-size:0.65rem;">Buy ' + (of.buy_qty || 1) + ' Get ' + (of.get_qty || 1) + '</span>';
                ...
                html += '<div class="mb-3 border rounded-3 overflow-hidden">'
                    ...
                    + (!isCancelled ? '<div class="px-3 py-2 text-end border-top small fw-semibold text-muted">Round Total: Rs ' + parseFloat(parseFloat(order.taxable_amount) - parseFloat(order.discount_amount)).toFixed(2) + '</div>' : '')
                    + '</div>';
            });
        }
        ...
        html += '<div class="p-3 bg-light border rounded-3 mt-2">'
            ...
            + '<div class="col-4 text-center fw-semibold text-success small">Rs ' + amount(walletBalance) + '</div></div>'
            + '</div>';
